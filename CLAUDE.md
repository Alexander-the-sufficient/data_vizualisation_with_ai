...
> **Everything you submit will be graded.** Additional work can _improve or reduce_ the grade — weak extras hurt. Do not pad.
...
14. _Optional:_ additional visualizations by you. Extras only at the end, and only if they add value (see grading note above).
...
- Task 7 (data-ink ratio) is a design discipline: strip every non-data mark. It is a concept to _apply rigorously_, not just nod at.
...
- Decide the story _before_ the chart type. The chart type serves the story, not the other way around.
...
- **Never double-encode** a single variable on two channels (e.g. color _and_ size for the same value) unless redundancy is deliberately needed for accessibility.
...
- **Every data point in the series must be visibly rendered.** A chart that _looks_ like it has gaps is perceived as missing data even when the underlying values are all present. Near-zero values that render invisibly count as missing perceptually.
...
- Apply **Shneiderman's mantra**: _overview first → zoom and filter → details on demand_. Do not open with details.
...
- Fix the scale _before_ drawing: units per square, axis range ending at a round number above the max.
...
### Common mistakes to avoid (lecture 06 — "The Ten Widest Mistakes")

A _mistake_ is a violation of a principle covered in class — never an accident. Either a wrong decision made without awareness, or no decision at all (default settings doing the deciding for you). Why mistakes matter: they mislead readers, undermine trust, and obscure the very insights you wanted to show. Going through the list below is a required pre-ship pass for every chart.

**#1 — Wrong scale.** Truncated Y-axis or mixing large and small numbers on one chart. Small differences end up looking enormous, large ones negligible. Bar height encodes magnitude — twice as tall ↔ twice the value. Fix: bar charts always start at zero (ratio variables); when combining series with very different scales, use **two separate charts** with their own scales — **never** a dual-axis chart on one set of axes (per instructor emphasis #8, this is non-negotiable; the lecture's "two Y-axes" wording is overridden by the stricter portfolio rule). Exception: line charts may cut the Y-axis if small changes matter and the context is clear.

**#2 — 3D charts.** Adding a third (depth) dimension to 2D data carries _zero information_, distorts lengths and angles via perspective projection, makes accurate value reading impossible, and breaks pre-attentive processing. Fix: don't use 3D effects. **Exceptions: none.** (The lecture mentions a "real 3D data" exception, but instructor emphasis #8 is unconditional and wins — this portfolio uses no 3D, period.)

**#3 — Pie chart abuse.** Pies fail when there are too many categories or when the question is _not_ parts-of-a-whole. Angle is a low-effective encoding (Mackinlay's hierarchy) and readers default to scanning for a majority. Fix: a pie must satisfy **both** constraints — purpose is part-to-whole **and** the existing 2–5-category cap from the chart-type-selection rules. Alternatives when either fails: sorted bar chart, treemap, stacked bar, waffle chart. For multiple yes/no questions, consider **multiple small pies** (small multiples), each still subject to the 2–5-category cap. Exceptions: none.

**#4 — Wrong chart for the task.** Chart type doesn't match the _nature of the data_ or the _task at hand_. Fix: return to the questions — what type of data? which story? — and apply the session-03 task taxonomy (composition, ranking, dispersion, correlation, …). Exceptions: rare.

**#5 — Wrong colours.** Colour scheme misrepresents the variable's nature, or is inaccessible — violates learned metaphors. Fix: gradual scale for continuous, qualitative palette for categorical, diverging palette for ordered variables with a midpoint. **Start from ColorBrewer schemes** — they are perceptually designed and tested. Exceptions: rare.

**#7 — Unnecessary boxes and grids.** Marks left over from the drawing process that distract the reader. Fix: for every element ask "can I remove it?" Exception: rarely, when separation is genuinely needed.

**#7 — Chartjunk.** Visual elements that add no information — decorative gridlines, background images, clipart, shadows, … Every non-data pixel competes with data pixels for attention; decoration is cognitive load. Fix: for every element ask "can I remove it?" Exceptions: never.

**#8 — Nothingburgers.** A great-looking visualization with no story (the canonical example: choropleth maps that effectively just show population density). Fix _before_ drawing: what is the story I want to tell? At which conclusion will the reader arrive? If neither answer is sharp, don't ship the chart — the reader's time is wasted searching in vain for a story that isn't there.

**#9 — Wrong annotations.** Raw labels straight from the data source, no annotations at all, missing/wrongly-ordered legend, inconsistent caption, default (alphabetical) ordering of categories on bar charts. Fix: review _all_ text elements as deliberately as the visual ones — not only the "viz", also every "text" element.

**#10 — Lack of consistency.** Sizes, colours, axis ranges varying across charts without reason. Reader cannot compare; cognitive load goes up. **AI-generated charts are especially prone** because each chart is generated in isolation. Fix: budget a **second and third pass over the entire portfolio** specifically for cross-chart consistency.

### Sonification (alternative encoding to know about)

Mapping data to sound parameters (pitch, volume, timbre, …). Useful as an **accessibility alternative** to visualization, and for revealing patterns in time-series data the eye misses (the ear is good at fractions/relations, rhythm, temporal patterns). Hard to learn to interpret. Resources: https://sonification.design, https://github.com/luebby/sonification. Not a portfolio task by itself, but worth knowing as a non-visual encoding option.

### Working with AI in 2026 (lecture 06 framing)

Course frame for _how_ this portfolio is meant to be built — not chart rules, but expectations on the human-AI workflow itself.

- **Accordion model.** AI compresses _Solve / Estimate / Code_ (the middle stage); it does **not** compress _Ideate / Generate hypothesis_ or _Verify / Communicate / Refine_. Those two ends expand to fill the time saved. Apply: spend the saved coding time on sharper questions and harder verification, not on more charts.
- **Future-of-data-science shifts** (more ↔ less): more **why** vs. less how; more (precise) **natural language** vs. less (algebraic) formalism; more **delegating** and **assessing** vs. less doing; more **reading**, **asking questions**, **oral communication** vs. less writing; **wider tool stack** vs. deeper single-tool expertise; **deployment** vs. implementation; **understanding of principles** vs. practice.
- **Job model.** The student's job in this course is increasingly _delegating tasks and approving PRs_ — i.e. specifying intent crisply, then critiquing what comes back. Defensibility (instructor emphasis #11) is the operational form of "approving a PR": every chart needs a reason you could state in one sentence.
- **Core challenges to be aware of.** Competent use of LLMs/agents; selection of appropriate model and mode; **adoption of an assessment culture**; clear formulation of intent (`README.md`, `AGENTS.md`, this `CLAUDE.md`); preparation of knowledge bases and datasets; deployment; cost management; creation of efficient skills; agent security.
- **Visualization-specific direction.** LLMs are strong with text and weak with the visual. The 2026 fixes are: better training on visualization code (better "mental model"), mental-model → visual feedback loops, draft plots → diffusion rendering, and direct diffusion rendering. Practical takeaway for this portfolio: **trust the LLM less on visual judgment than on code/data wrangling** — verify every render with your own eyes (and against the ten-mistakes list above).
...
- **Heritage Red `#D92B2B`** is reserved for source-URL links, single callouts marking _the moment_ of a chart's story, and rare highlights of _extreme_ data points (e.g. the SNB trough, the four worst glacier years). Never a fill for more than ~10% of the chart's ink.
...
1. **Every chart's data comes from a downloaded source file. No hand-typed numbers, ever.**
   ...
   - **Never** transcribe numbers from a webpage, PDF, news article, or memory into a `tribble()`, `c(...)`, or any in-script literal. If the source publishes only an HTML table, scrape it to a CSV first and commit the CSV — the _script reads the CSV_. If the source is a public API, save the raw API response (JSON) to `data/task_<n>/` and read from disk; do not bake API values into the script.
   - **Annotation overlays are exempt** — `tribble()` is OK for label text, event names, callout positions, and other editorial overlays that exist for visual storytelling. Any _measured value_ (counts, amounts, rates, percentages, dates of observations) must come from the file.
...
2. **Claude does NOT draft or outline the written text** for tasks 1 (critique), 3 (critique), 13 (process note), 15, 16, 17 (tool / inspiration write-ups). These are fully human-written, as mandated by the brief — Claude provides no drafts, outlines, or bullet points for them.
