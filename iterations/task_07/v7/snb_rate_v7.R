# Task 7 v7 — SNB policy rate, 2000-2026 (Tufte data-ink rigor).
# v7 fixes a factual/annotation error in v6: v6 labelled the 2007 point
#   (2.75%) as the "pre-GFC peak", but the actual maximum of the plotted
#   series (x-axis starts 2000) is 3.50%, held Jun 2000 - Jan/Feb 2001 —
#   visibly the highest point of the line, and sitting to the LEFT of the
#   2007 point. The console already printed `peak: 3.5`. v6 thus labelled
#   a lower point as "the peak" while the true maximum sat unlabelled.
#   Fix: the labelled pre-GFC HIGH is now the true series maximum, 3.50%,
#   annotated at the plateau midpoint (~2000-10). The 2007 point (2.75%)
#   is kept as a clearly-secondary landmark, plainly labelled "2007: 2.75%"
#   — it is no longer called "the peak". Everything else from v6 is kept:
#   the data-ink-minimal sparkline (no axes/ticks/gridlines/border/legend),
#   the -0.75% trough as the single heritage-red accent, the 1.75% (2023)
#   recovery landmark, and the 0.00% endpoint.
#
...
ann <- tibble::tribble(
  ...
  peak_date,               3.50,  "3.50% pre-GFC high\nJun 2000",  peak_date,                 3.62,   "peak_above",
  as.Date("2007-09-15"),   2.75,  "2007: 2.75%",                   as.Date("2008-07-15"),     2.95,   "landmark_below",
...
gap_y <- 0.18
leaders <- ann %>%
  filter(role %in% c("event_above", "trough", "peak_above", "landmark_below")) %>%
...
p <- ggplot(policy, aes(x = date, y = rate)) +
  ...
  geom_text(data = ann %>% filter(role == "peak_above"),
            ...
            hjust = 0.5, vjust = 0,
            lineheight = 0.95) +
  geom_text(data = ann %>% filter(role == "landmark_below"),
            aes(x = lbl_x, y = lbl_y, label = label),
  ...
  scale_y_continuous(limits = c(-2.0, 4.1),
...
out_pdf <- "iterations/task_07/v7/snb_rate_v7.pdf"
