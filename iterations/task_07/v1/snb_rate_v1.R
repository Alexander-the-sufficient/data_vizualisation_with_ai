# Task 7 v1 — SNB policy rate, 2000-2026 (Tufte data-ink rigor).
# Sparkline-style: no axes, no ticks, no gridlines, no border, no legend.
# The line + 5 annotations carry every piece of information.
...
ann <- tibble::tribble(
  ...
  as.Date("2014-12-15"),  -0.25,  "NIRP introduced\nDec 2014",  as.Date("2014-12-15"),     1.20,   "event_above",
  as.Date("2015-01-15"),  -0.75,  "-0.75% trough\nJan 2015",    as.Date("2015-02-15"),    -1.55,   "trough",
  as.Date("2022-09-15"),   0.50,  "Exit\nSep 2022",             as.Date("2022-09-15"),     2.10,   "event_above",
...
gap_y <- 0.18
leaders <- ann %>%
  filter(role %in% c("event_above", "trough")) %>%
...
out_pdf <- "iterations/task_07/v1/snb_rate_v1.pdf"
