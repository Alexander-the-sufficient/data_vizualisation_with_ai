# Task 8 v19 — Partners Group 2024 income statement Sankey, networkD3.
#
# v19 changes vs v18:
#   * Widget width 1280 -> 1600 px. v18's Apple-style layout was right
#     but the inner cascade subtotal labels (Revenue / Operating margin /
#     Pre-tax profit / Net profit / Dividend) collided horizontally
#     because the cascade columns were too close together. The wider
#     canvas gives each column ~265 px instead of ~180 px, enough to
#     fit "Operating margin  CHF 1.48 bn (+13% Y/Y)" between columns
#     without overlap.
#   * Inner cascade subtotals (Revenue, Operating margin, Pre-tax
#     profit, Net profit, Dividend) labels MOVED ABOVE their stratum
#     bars via a JS callback, anchored centred on the bar's x-position.
#     Sources stay outer-left, cost peels stay right of their bars.
#   * webshot crops to the actual SVG bounding box (vheight matches the
#     widget height) so the resulting PDF has no white band below the
#     chart.
#
# v18 changes vs v17:
#   * sinksRight = FALSE so terminal cost peels stop at their natural
#     depth column instead of being jammed against the right edge.
#   * iterations = 32 restored (d3-sankey relax pass). Combined with
#     the manual node ordering (profit listed first at each column,
#     costs after), this minimises crossings while honouring the
#     vertical hierarchy.
...
sn <- sankeyNetwork(
  ...
  width        = 1600,
  height       = 640,
  margin       = list(top = 60, right = 240, bottom = 24, left = 240)
)
...
sn <- htmlwidgets::onRender(sn, "
  function(el, x) {
    ...
    // Find min and max node x positions (source column / sink column).
    var xs = [];
    ...
    // Inner cascade subtotals — labels ABOVE their stratum bars.
    var aboveBar = ['Revenue', 'Operating margin', 'Pre-tax profit',
                    'Net profit', 'Dividend'];
    ...
    d3.select(el).selectAll('.node text').each(function(d) {
      ...
      if (aboveBar.indexOf(name) !== -1) {
        // Re-anchor: centred horizontally over the node, lifted above
        // the stratum bar's top edge by 8 px.
        t.attr('x', d.dx / 2)
         .attr('y', -8)
         .attr('dy', '0')
        ...
        // Source column: labels left of node, right-aligned.
        t.attr('x', -6)
         .attr('text-anchor', 'end');
      }
      // Cost-peel labels keep networkD3's default (right of node).
    });
...
html_out <- "iterations/task_08/v19/pg_income_sankey_v19.html"
pdf_out  <- "iterations/task_08/v19/pg_income_sankey_v19.pdf"
