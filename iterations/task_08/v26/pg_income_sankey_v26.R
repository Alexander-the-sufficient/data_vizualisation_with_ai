# Task 8 v26 — Partners Group 2024 income statement Sankey, networkD3.
#
# v26 changes vs v25 (palette refresh — v25's source=alloy +
# cost=dark_stone were both warm grays separated by only ~16% L*,
# so the chart read as one big muddy gray flow with copper accents):
#   * source  -> dark_quartz `#ACA39A` (soft taupe — informational
#     inflow, recedes; was alloy `#5C5B59`)
#   * cost    -> alloy `#5C5B59` (deep warm charcoal — gives the
#     cost branch real weight; was dark_stone `#7E8182`)
#   * profit  -> copper `#896C4C` (unchanged — the story accent)
#   * retained-> light_quartz `#ECEAE4` (unchanged — sliver)
#   * Revenue inside-bar label switches from white to alloy text
#     since white on dark_quartz fails contrast (~2.7:1). All other
#     inside-bar labels (Operating costs / Operating margin /
#     Pre-tax profit / Net profit) keep white text — their bars are
#     dark enough (alloy / copper).
#
# All other layout (upward staircase on profit chain, link path
# recomputation, margins, label placement) is identical to v25.
#
...
colour_scale <- sprintf(
  paste0("d3.scaleOrdinal()",
         ...
         ".range(['%s','%s','%s','%s'])"),
  pg_palette$dark_quartz,    # source — soft taupe (was alloy)
  pg_palette$copper,         # profit — copper (unchanged)
  pg_palette$alloy,          # cost   — deep charcoal (was dark_stone)
  pg_palette$light_quartz    # retained — cream (unchanged)
)
  ...
  function(el, x) {
    ...
    d3.select(el).selectAll('.node text').each(function(d) {
      ...
      if (insideBar.indexOf(name) !== -1) {
        // Revenue sits on dark_quartz (#ACA39A) — too light for white
        // text. All other inside-bar nodes are on alloy or copper, both
        // dark enough for white.
        var labelFill = (name === 'Revenue') ? '#5C5B59' : '#FFFFFF';
        t.text('');
        t.attr('x', d.dx / 2)
         .attr('y', d.dy / 2)
...
html_out <- "iterations/task_08/v26/pg_income_sankey_v26.html"
pdf_out  <- "iterations/task_08/v26/pg_income_sankey_v26.pdf"
