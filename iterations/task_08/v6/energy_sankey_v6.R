# Task 8 v6 — US Primary Energy Flow, 2023 (canonical LLNL Sankey).
#
# Changes from v5:
...
#     the end-use column they re-converge from both routes. v5 did the
#     electricity routing as invisible math; v6 shows it as a chart
#     node, which is what makes the LLNL diagram a textbook example.
#   * Four ggalluvial axes: Source → Conversion (Electricity gen | Direct
#     use) → End-use (sectors + Generation loss) → Disposition (Useful |
#     Rejected). Bands keep their disposition colour along the entire
#     path so the rejected-energy story reads at every stage.
#
...
out_pdf <- "iterations/task_08/v6/energy_sankey_v6.pdf"
