# Task 8 v9 — US Primary Energy Flow, 2023 (efficiency-coloured rebuild).
# v9 changes vs v8:
#   * White stratum outlines (color = "white", linewidth = 0.6) so the
#     vertical separation between stacked strata is visually obvious
#     instead of relying on hairline antialiasing seams. Reads like
#     defined "boxes" rather than smeared bands.
#   * Generation loss explicitly coloured copper (the "rejected" hue)
#     instead of the v8 default of alloy. The translucent alluvium
#     flowing through Generation loss is 100% rejected, which tinted
#     the alloy bar visibly bronze — making it look inconsistent vs
#     the four real end-use sectors (Industrial, Transportation,
#     Residential, Commercial) which sit alloy. Generation loss is
#     structurally NOT an end-use sector — it's the conversion-stage
#     waste branch where 100% of the energy ends up as Rejected. Making
#     it visibly copper acknowledges that semantically: copper means
#     "rejected at this stage," whether at Generation loss (early waste)
#     or at the right-most Rejected Energy total (final disposition).
#   * Alluvium alpha lowered slightly (0.7 -> 0.55) so the white-edged
#     strata read as the chart's structural grid rather than fighting
#     the flows for attention.
#
# v8 changes vs v7:
#   * Disposition recolouring under the new "red and black are accents
#     only" colour discipline. v7 used `heritage_red` (then sage) for
#     "Useful Energy"; under the v4 palette that token now resolves to
#     bright PG red, which would paint ~33% of the chart's flow ink red
#     — way past the accent threshold. v8 uses `dark_stone` (warm gray)
#     for Useful Energy: a calm, neutral colour that says "this part of
#     the system is fine," letting the warm bronze (Rejected) carry the
#     story. Stratum bars switched from `onyx` (pure black) to `alloy`
#     (warm charcoal) so the structural bars recede slightly and the
#     coloured flows pop.
#
...
# ---- Colours (semantic) -----------------------------------------------
disp_colors <- c(
  "Useful Energy"   = pg_palette$dark_stone,    # warm gray — calm "this is fine"
  "Rejected Energy" = pg_palette$copper         # warm bronze — heat lost
)
# All strata are warm charcoal (alloy, not pure onyx — softens the
# structural bars so the coloured flows are the focus). The two
# disposition strata pick up the semantic colour so the right column
# reads as the chart's climax.
all_strata <- c(src_order, conv_order, end_order, disp_order)
stratum_fill <- setNames(rep(pg_palette$alloy, length(all_strata)),
                         all_strata)
stratum_fill["Useful Energy"]    <- pg_palette$dark_stone
stratum_fill["Rejected Energy"]  <- pg_palette$copper
# Generation loss is 100% rejected energy — colour it copper to match
# the disposition-encoding rule, not alloy like the real end-use sectors.
...
disp_labels <- disp_pos %>%
  transmute(x = 4 + stratum_w / 2 + 0.04, y = y_mid,
            ...
            color = if_else(stratum == "Useful Energy",
                            pg_palette$dark_stone, pg_palette$copper))
...
out_pdf <- "iterations/task_08/v9/energy_sankey_v9.pdf"
