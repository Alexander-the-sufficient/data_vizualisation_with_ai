# Task 8 v14 — Partners Group 2024 income statement Sankey, Apple-style
# (label cleanup pass).
#
# v14 changes vs v13:
#   * Source and cost-peel labels switched from 3-line ("Name / CHF X /
#     +Y% Y/Y") to compact 1-line format ("Name  CHF X  +Y% Y/Y") so
#     adjacent small strata don't pile labels on top of each other. The
#     four prominent inner subtotals (Revenue, Operating margin, Pre-tax
#     profit, Net profit) and the two terminal allocations (Dividend,
#     Retained) keep their multi-line emphasis.
#   * Smallest cost peels (D&A, Finance expense) get LEADER LINES from
#     their stratum out to labels in clearer space below the chart, so
#     the three crowded peels at axis 5 (Tax / D&A / Finance expense)
#     no longer overlap each other.
#   * Smallest sources (Other op income, Finance income) likewise get
#     leader lines from their stratum out to labels below the chart.
#
# 6 axes (unchanged from v13):
#   1: Leaf revenue sources (4)
#   2: Revenue
#   3: Operating margin (copper) / Operating costs
#   4: Pre-tax profit / Other costs / Personnel / Other op exp
#   5: Net profit / Tax / D&A / Finance expense
#   6: Dividend / Retained
#
# Data source: Partners Group Holding AG, Annual Report 2024,
#   Consolidated statement of profit or loss (page 39).
#   CSV: data/task_08_pg/pg_income_statement_2024.csv
...
gap_top  <- top_y * 0.025
gap_bot  <- top_y * 0.06   # space below the chart for leader-line labels

# Inner subtotals (Revenue, Operating margin, Pre-tax profit, Net profit):
# 3-line label ABOVE the stratum bar, in alloy.
...
# Compact 1-line label format used by sources, cost peels, and the
# small terminal items (Retained). Bullet separator for compactness.
fmt_compact <- function(name, count) {
  yoy_str <- sapply(name, fmt_yoy)
  ifelse(yoy_str == "",
         sprintf("%s  %s", name, fmt_chf(count)),
         sprintf("%s  %s  %s", name, fmt_chf(count), yoy_str))
}

# Source labels at axis 1: Mgmt fees and Performance fees inline left;
# the two small streams (Other op income, Finance income) get LEADER
# LINES from their stratum out to labels below the chart so they
# don't overlap each other.
ax1_inline <- strat_data %>%
  filter(x == 1, as.character(stratum) %in% c("Mgmt fees",
                                              "Performance fees")) %>%
  mutate(
...
# Leader-line labels for the two small sources, anchored below the panel.
ax1_leader_data <- strat_data %>%
  filter(x == 1, as.character(stratum) %in% c("Other op income",
                                              "Finance income")) %>%
  mutate(
    x_anchor = 1 - stratum_w / 2,            # right edge of leader at stratum
    y_anchor = y_mid,
    x_lbl    = 1 - stratum_w / 2 - 0.10,
    y_lbl_offset = case_when(
      as.character(stratum) == "Other op income" ~ -gap_bot * 1.4,
      as.character(stratum) == "Finance income"  ~ -gap_bot * 2.6,
      TRUE                                       ~ 0
    ),
    y_lbl = y_lbl_offset,                    # absolute y near bottom
    label = fmt_compact(as.character(stratum), count),
    color = pg_palette$alloy,
    face  = "plain"
  )

# Cost peels: 1-line labels right of stratum. The three small peels at
# axis 5 (Tax / D&A / Finance expense) get leader lines into the
# bottom margin so they don't overlap.
...
# Tax label: inline (it's the biggest of the three at axis 5)
tax_inline <- strat_data %>%
  filter(x == 5 & stratum == "Tax") %>%
  mutate(
    x_lbl = 5 + stratum_w / 2 + 0.05,
    y_lbl = y_mid,
    label = fmt_compact(as.character(stratum), count),
    color = pg_palette$alloy,
    face  = "plain"
...
# Leader-line labels for D&A and Finance expense (smallest peels).
peel_leader_data <- strat_data %>%
...
# Final allocation labels at axis 6: Dividend (3-line bold copper),
# Retained (compact, alloy).
...
# Leaders (drawn first so labels sit on top).
if (nrow(ax1_leader_data) > 0) {
  p <- p +
    geom_segment(data = ax1_leader_data,
                 aes(x = x_anchor, xend = x_lbl + 0.02,
                     y = y_anchor, yend = y_lbl + top_y * 0.012),
                 color = pg_palette$alloy, linewidth = 0.2,
                 inherit.aes = FALSE)
}
if (nrow(peel_leader_data) > 0) {
  p <- p +
    geom_segment(data = peel_leader_data,
                 aes(x = x_anchor, xend = x_lbl - 0.02,
                     y = y_anchor, yend = y_lbl + top_y * 0.012),
                 color = pg_palette$alloy, linewidth = 0.2,
                 inherit.aes = FALSE)
...
p <- add_text(p, ax1_leader_data,   hjust = 1)
p <- add_text(p, inner_subtotals,   hjust = 0.5, vjust = 0)
p <- add_text(p, cost_peels_inline, hjust = 0)
p <- add_text(p, tax_inline,        hjust = 0)
p <- add_text(p, peel_leader_data,  hjust = 0)
p <- add_text(p, ax6_dividend,      hjust = 0)
p <- add_text(p, ax6_retained,      hjust = 0)

out_pdf <- "iterations/task_08/v14/pg_income_sankey_v14.pdf"
