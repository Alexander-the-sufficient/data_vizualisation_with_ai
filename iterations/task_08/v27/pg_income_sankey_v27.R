# Task 8 v27 — Partners Group 2024 income statement Sankey, networkD3.
#
# v27 changes vs v26:
#   * cost group switches from alloy to heritage_red_dk `#830011`
#     (deep crimson — Apple-FY22-style "money lost" semantics).
#     NOTE: this exceeds design_system.md's "red <= 10% of ink"
#     guideline (cost flows are ~35% of total flow ink). Logged in
#     reference_log.md as a deliberate, user-approved deviation —
#     the income-statement convention (red = expense) is strong
#     enough to justify the override here.
#   * Cascade subtotal labels (Revenue / Operating margin / Pre-tax
#     profit / Net profit) moved OUT of their bars and placed ABOVE
#     each bar. Operating costs's label placed BELOW its bar. The
#     v22-v26 inside-bar Apple treatment covered the bars and bled
#     onto the surrounding flows, which the user explicitly rejected.
#   * All cascade labels in alloy text on white background; multi-line
#     tspan (name / value / yoy) preserved.
#   * Margins bumped: top 80 -> 120, bottom 24 -> 60 to make room for
#     the above/below-bar labels (Net profit is shifted up 75 by the
#     staircase, plus a 3-line label needs another ~40 px above its bar).
#
...
# bold-copper outside-right) are unchanged from v25/v26.
#
...
colour_scale <- sprintf(
  paste0("d3.scaleOrdinal()",
         ...
         ".range(['%s','%s','%s','%s'])"),
  pg_palette$dark_quartz,        # source   — soft taupe
  pg_palette$copper,             # profit   — warm bronze
  pg_palette$heritage_red_dk,    # cost     — deep crimson (#830011)
  pg_palette$light_quartz        # retained — cream sliver
)
...
html_out <- "iterations/task_08/v27/pg_income_sankey_v27.html"
pdf_out  <- "iterations/task_08/v27/pg_income_sankey_v27.pdf"
