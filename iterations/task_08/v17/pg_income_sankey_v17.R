# Task 8 v17 — Partners Group 2024 income statement Sankey, networkD3.
#
# v17 changes vs v16:
#   * iterations = 0  -> d3-sankey honours the manual node order
#     instead of running its own crossing-minimisation. This pins the
#     profit cascade (Operating margin / Pre-tax profit / Net profit /
#     Dividend) along the top edge of the chart and the cost peels
#     (Operating costs / Personnel / Other op exp / Other costs / Tax /
#     D&A / Finance expense) along the bottom, exactly like the
#     Apple FY22 reference. v16 left the order to the optimiser, which
#     pushed Operating costs to the TOP of the second column — wrong
#     visual hierarchy.
#   * Source labels relocated to the OUTSIDE LEFT edge of the chart via
#     a small post-render JavaScript callback. networkD3's default puts
#     labels to the right of every node, which on the leftmost column
#     means they sit between the source nodes and Revenue, fighting
#     the alluvium. Apple-style is labels in the page margin.
#   * Widget aspect ratio tightened (1280 x 600 instead of 1280 x 720)
#     and webshot crops to the rendered widget so the resulting PDF
#     fills the page horizontally without the empty band below.
#   * Inner subtotal labels (Revenue, Operating margin, Pre-tax profit,
#     Net profit, Dividend) are bolded via the JS callback so they
#     read as cascade markers rather than peer labels.
#
...
html_out <- "iterations/task_08/v17/pg_income_sankey_v17.html"
pdf_out  <- "iterations/task_08/v17/pg_income_sankey_v17.pdf"
