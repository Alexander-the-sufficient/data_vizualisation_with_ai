# Task 11 v13 — Mean adult standard health-insurance premium by canton, 2026.
#
# v13 vs v11: removed the two annotate("text") calls that drew the
# "76% gap" red headline and the "Genève vs. Zug, same federal mandate."
# subtitle inside the chart panel. Both texts duplicated information
# already carried by the Quarto page title above the chart — a violation
# of the project's separation-of-concerns rule (title, subtitle, and
# source citation live in Quarto, not in the R chart). v13 keeps only
# the two extreme-canton leader labels (GE / ZG) and the legend; the
# headline "76% gap" story lives in the Quarto title only.
#
# Topic background: v12 was a topic pivot to a US state-level FHFA HPI
# choropleth via FRED, exploring whether the Northeast-leads-housing-boom
# story would serve the portfolio better than the Swiss premium story.
# Reverted on review — Swiss subject matter sits better alongside the
# portfolio's other Swiss/Partners-Group charts (T7 SNB rate, T8 PG
# income statement). v12 left on disk under `iterations/task_11/v12/`
# as a documented pivot-and-revert experiment.
#
...
nudges <- tribble(
  ...
  "ZG",    115e3,     68e3     # NE — between v7 (too close) and v10
                                #     (too far): comfortable gap from
                                #     the NE edge of the country
)
...
ch_bbox <- st_bbox(ch)
xlim <- c(ch_bbox$xmin - 90e3, ch_bbox$xmax + 115e3)   # extra E room
ylim <- c(ch_bbox$ymin - 80e3, ch_bbox$ymax + 90e3)    # extra N room
...
out_pdf <- "iterations/task_11/v13/kvg_premium_v13.pdf"
