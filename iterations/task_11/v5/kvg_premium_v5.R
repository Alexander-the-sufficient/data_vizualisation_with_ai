# Task 11 v5 — Mean adult standard health-insurance premium by canton, 2026.
#
# v5 vs v4:
#   * Panel canvas. v4 left coord_sf at the country's bbox, so any nudge
#     pointing N (Basel-Stadt) or SW (Genève) clipped at the page edge.
#     v5 sets explicit xlim/ylim with ~80 km whitespace each side and
#     `clip = "off"` so callouts breathe into the surrounding margin.
#   * Callout count. v4 used six callouts (GE/TI/BS top, ZG/AI/NW bottom).
#     v5 keeps only four: the two extremes (GE highest, ZG lowest) plus
#     BS and AI as evidence of the pattern (urban high vs. rural low).
#     Removing TI and NW reduces line crossings and lets the colour ramp
#     do the rest of the work.
#   * geom_label. v4 used geom_text against canton fills; v5 uses
#     geom_label with white background and no border so the label reads
#     cleanly even where it overlaps a polygon.
#   * Headline call-out. v5 adds a single inline annotation in the
#     bottom-left whitespace stating the 76% Genève↔Zug gap — the
#     headline of the story, where the chart's narrative lives.
#
...
# Compute the headline ratio for the inline annotation.
...
# Nudges in LV95 metres (Switzerland is ~350 × 220 km).
# Each label sits in nearby whitespace; segments stay short and honest.
nudges <- tribble(
  ~Kanton, ~nudge_x, ~nudge_y,
  "GE",    -50e3,    -55e3,    # SW into French border whitespace
  "ZG",     90e3,     50e3,    # NE through Schaffhausen direction into N whitespace
...
# Country bbox + manual margin so callouts have room.
ch_bbox <- st_bbox(ch)
...
p <- ggplot(ch) +
  ...
  # Headline gap callout — bottom-left whitespace.
  annotate("text",
           x = xlim[1] + 5e3, y = ylim[1] + 18e3,
           label = sprintf("76%% gap"),
           hjust = 0, vjust = 0,
           family = chart_family, size = 6.5,
           color = pg_palette$heritage_red,
           fontface = "bold") +
  annotate("text",
           x = xlim[1] + 5e3, y = ylim[1] + 5e3,
           label = "between Genève and Zug, same federal mandate",
           hjust = 0, vjust = 0,
           family = chart_family, size = 3.0,
           color = pg_palette$alloy) +
...
out_pdf <- "iterations/task_11/v5/kvg_premium_v5.pdf"
