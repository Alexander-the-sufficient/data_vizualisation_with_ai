# Task 4 v3 — Swiss glacier annual mass balance, 1956-2025.
# v3 changes vs v2: highlight color switched from copper (warm bronze) to
#   heritage_red (#D92B2B PG brand red). With the v4 palette migration
#   "copper" is now a muted bronze rather than v2's saturated terracotta,
#   which dampened the visual emphasis on the four catastrophic years.
#   Heritage red is the right call here under the new colour discipline:
#   four bars out of seventy is ~5.7% of the chart's ink — true accent
#   ratio, not "thick red graph" — and the data semantically matches red
#   (extreme negative outliers, "danger" framing of accelerating ice loss).
# v2 changes vs v1: highlight color switched from heritage_red (then deep
#   sage, near-identical lightness to alloy) to copper (terracotta) so
#   the four 2022-2025 bars actually contrast against the rest of the
#   series. v3 reverts to heritage_red after the palette migration made
#   it a true red.
# Story: the past four years (2022-2025) lost more ice than the entire
#   34-year period 1956-1989 (−7.6 vs −6.0 m water equivalent).
#
...
# Highlight the past four hydrological years (2022-2025)
swiss <- swiss %>%
...
p <- ggplot(swiss, aes(x = hydro_year, y = Ba_plot, fill = highlight)) +
  ...
  scale_fill_manual(
    values = c(`FALSE` = pg_palette$alloy, `TRUE` = pg_palette$heritage_red),
    guide  = "none"
  ) +
  scale_x_continuous(
...
out_pdf <- "iterations/task_04/v3/glacier_mass_balance_v3.pdf"
