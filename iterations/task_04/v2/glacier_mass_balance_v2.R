# Task 4 v2 — Swiss glacier annual mass balance, 1956-2025.
# v2 changes vs v1: highlight color switched from heritage_red (deep sage,
#   near-identical lightness to alloy) to copper (terracotta) so the four
#   2022-2025 bars actually contrast against the rest of the series.
...
p <- ggplot(swiss, aes(x = hydro_year, y = Ba_plot, fill = highlight)) +
  ...
  scale_fill_manual(
    values = c(`FALSE` = pg_palette$alloy, `TRUE` = pg_palette$copper),
    guide  = "none"
  ) +
  scale_x_continuous(
...
out_pdf <- "iterations/task_04/v2/glacier_mass_balance_v2.pdf"
