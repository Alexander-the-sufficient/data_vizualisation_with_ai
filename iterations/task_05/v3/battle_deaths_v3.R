# Task 5 v3 — Global battle-related deaths, 1989-2024 (B&W).
# v3 changes vs v2:
#   * Restored the 1990 "Gulf War / Liberia / Sri Lanka" callout. v2
#     dropped it (argued the bump was small vs later peaks and the label
#     was anchoring on a flat shoulder, not a real local maximum); v3
#     puts it back so the four roughly-equidistant decades each get one
#     regional context label.
#   * Repositioned the 1999 leader. v2 had `x_lbl = 2003` which floated
#     the label far to the right of the actual peak at x = 1999, leaving
#     the leader long and visually awkward. v3 uses `x_lbl = 2001`, a
#     short two-year offset that keeps the label close to the data point
#     it references.
# v2 changes vs v1: title rewording in Quarto (UCDP records start in 1989,
#   so the original "any year since records began" was scoped too broadly).
...
ann <- tibble::tribble(
  ...
  ann_pos <- ann
  ann_pos$x_lbl <- c(1993,   2001,   2010,   2017)
  ann_pos$y_lbl <- c(135000, 110000, 150000, 285000)
...
out_pdf <- "iterations/task_05/v3/battle_deaths_v3.pdf"
